...
| 视频全屏 | 横竖屏；退出回详情壳 |
| 独立 LLM 对话 | `audio` / `video` **不**触发网友评论任务；隐藏评论入口，改为仅媒体详情可见的「一起聊」悬浮窗 |
...
- **音频**：后台 + mini + 通知  
- **视频**：详情内 / 全屏；切走 **暂停**；mini 仅显示「已暂停 · 点按回详情」（或首版视频不进 mini，减少复杂度）  

产品若坚持视频也 mini 画中画：二期 `pip` / 系统 PiP，不进 P1 必做。
